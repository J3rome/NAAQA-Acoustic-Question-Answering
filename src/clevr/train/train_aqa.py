...
# FIXME : Don't hardcode folder name
tokenizer = CLEVRTokenizer(os.path.join(args.data_dir, 'preprocessed', config["dico_name"]))

# Load data
logger.info('Loading data..')
trainset = AQADataset(args.data_dir, which_set="train", image_builder=image_builder)
validset = AQADataset(args.data_dir, which_set="val", image_builder=image_builder)
testset = AQADataset(args.data_dir, which_set="test", image_builder=image_builder)
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options, allow_soft_placement=True)) as sess:
    ...
    if use_resnet:
        # FIXME : Don't hardcode folder name
        resnet_saver.restore(sess, os.path.join(args.data_dir, 'resnet', 'resnet_v1_{}.ckpt'.format(resnet_version)))
