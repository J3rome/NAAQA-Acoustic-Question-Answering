...
class DAQA_dataset(Dataset):
    ...
    def __init__(self, folder, version_name, input_image_type, set_type, questions=None, transforms=None,
        ...
        # FIXME : Those might not translate correctly
        ...
        if os.path.isfile(scene_file_path):
            scenes_def = {int(s['audio_index']): s for s in read_json(scene_file_path)['narratives']}
        else:
            scenes_def = None
        ...
        for i, sample in enumerate(questions):
            ...
            scene_id = int(sample["audio_index"])
            scene_filename = sample['audio_filename']
            ...
            if scene_id not in self.scenes:
                ...
                if scenes_def:
                    self.scenes[scene_id]['definition'] = scenes_def[scene_id]
                    self.scenes[scene_id]['definition']['objects'] = self.scenes[scene_id]['definition']['events']
                    del self.scenes[scene_id]['definition']['events']
                    self.scenes[scene_id]['definition']['duration'] = int(sum(o['duration'] for o in scenes_def[scene_id]['objects']) * 1000)

            game = {
                'id': question_id,
                'image': {'id': scene_id, 'filename': scene_filename, 'set': self.set},
                'question': question,
                'answer': answer
            }

            self.scenes[scene_id]['question_idx'].append(question_id)
    ...
    def from_dataset_object(cls, dataset_obj, games=None):
        ...
        if games:
            ...
            for idx, game in games.items():
                question = {
                    ...
                    'audio_index': game['image']['id'],
                    'audio_filename': game['image']['filename'].replace('.png', '.wav'),
        ...
        return cls(folder_path, version_name=dataset_obj.version_name,
                   ...
                   use_cache=dataset_obj.use_cache, do_transforms_on_device=dataset_obj.do_transforms_on_device,
                   preprocessed_folder_name=dataset_obj.preprocessed_folder_name)
    ...
    def get_spectrogram_transform_infos(self,):
        ...
        for transform in self.transforms.transforms:
            if type(transform) is GenerateSpectrogram:
                return transform.n_fft, transform.hop_length, transform.keep_freq_point, None
            elif type(transform) is GenerateMelSpectrogram:
                return transform.n_fft, transform.hop_length, transform.keep_freq_point, transform.n_mels
