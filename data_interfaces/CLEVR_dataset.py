...
class CLEVR_dataset(Dataset):
    ...
    def __init__(self, folder, version_name, input_image_type, set_type, questions=None, transforms=None,
        ...
        if questions is None:
            question_file_path = '{}/questions/CLEVR_{}_questions.json'.format(self.root_folder_path, self.set)

            questions = read_json(question_file_path)["questions"]

        scene_file_path = '{}/scenes/CLEVR_{}_scenes.json'.format(self.root_folder_path, self.set)
        if os.path.isfile(scene_file_path):
            scenes_def = {int(s['image_index']): s for s in read_json(scene_file_path)['scenes']}
        else:
            scenes_def = None
